        ...
        if rotor is None:
            ...
            if ROSS_AVAILABLE and st.button("🔧 Charger un rotor exemple (compresseur)", use_container_width=True):
                with st.spinner("Chargement..."):
                    try:
                        ...
                        st.error(f"Erreur : {e}")
            # Pas de return - on continue l'exécution
        else:
            # Onglets principaux
            ...
            with tab_stat:
                ...
                if static is not None:
                    ...
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        if hasattr(static, 'shaft_deflection') and static.shaft_deflection is not None:
                            max_deflection = max(abs(static.shaft_deflection))
                            st.metric("Flèche maximale", f"{max_deflection*1e6:.2f} µm")
                        else:
                            st.metric("Flèche maximale", "N/A")
                    with col2:
                        if hasattr(static, 'bearing_forces') and static.bearing_forces is not None and len(static.bearing_forces) > 0:
                            st.metric("Réaction palier gauche", f"{static.bearing_forces[0,0]/1000:.1f} N")
                        else:
                            st.metric("Réaction palier gauche", "N/A")
                    with col3:
                        if hasattr(static, 'bearing_forces') and static.bearing_forces is not None and len(static.bearing_forces) > 0:
                            st.metric("Réaction palier droit", f"{static.bearing_forces[-1,0]/1000:.1f} N")
                        else:
                            st.metric("Réaction palier droit", "N/A")
                    ...
                    try:
                        if selected_plot == "Déformée de l'arbre":
                            if hasattr(static, 'plot_deflection'):
                                fig = static.plot_deflection()
                            elif hasattr(static, 'plot_deformation'):
                            ...
                            elif hasattr(static, 'plot_shearing_force'):
                        ...
                        if fig is not None:
                            ...
                            st.info("Visualisation non disponible dans cette version de ROSS")
                    except Exception as e:
                        st.info(f"Visualisation non disponible : {e}")
            ...
            with tab_modal:
                ...
                if modal is not None:
                    ...
                    if hasattr(modal, 'wd') and modal.wd is not None:
                        wd_hz = modal.wd / (2 * np.pi)
                    else:
                        wd_hz = wn_hz
                    
                    if hasattr(modal, 'log_dec') and modal.log_dec is not None:
                        log_dec = modal.log_dec
                    else:
                        log_dec = np.zeros(len(wn_hz))
                    ...
                    try:
                        ...
                        st.info(f"Visualisation du mode {mode_idx+1} non disponible : {e}")
